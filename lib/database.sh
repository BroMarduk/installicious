...
# the feature-database-mariadb child. Historically there was also a
# feature-database-mysql child; it was removed after we confirmed that
# on Debian Bookworm/Trixie `default-mysql-server` resolves to
# mariadb-server, so the two options were always installing the same
# thing. The wrapper kept the name "mysql_family" so a future Oracle
# MySQL repo could plug in via the same helper without restructuring.
#
# The child is a thin shell that sources this lib and calls:
#   database_install_mysql_family "$II_ID" "$II_APT_PACKAGES" "$DB_TYPE"
#       "$SERVICE" "$STATUS_FILE"
#   database_uninstall_mysql_family ... (same args)
#
# Where DB_TYPE is "mariadb" today (also accepted: "mysql" — legacy
# value still appearing in pre-2.7.20 installs' database.state files)
# and SERVICE is the systemd unit name. The child supplies only the
# differences
...
# Top-level helper called by feature-database-mariadb (and historically
# by feature-database-mysql before that leaf was removed in 2.7.20).
...
# Generated by feature-database-mariadb when DATABASE_INNODB_TUNE=on.
