...
II_EDITABLE_CONFIG="WEEWX_DB_DIR WEEWX_DB_HDD_DIR WEEWX_DB_ROTATIONS"
# === II_MANIFEST_END ===
...
# Generated by feature-weewx-database-ramdisk.sh for ${DETECTED_PI_MODEL}
...
do_install() {
  if status_should_skip "$II_ID" "$II_VERSION" "$FILE_CONFIG_WEEWX"; then
    log_info "weewx-database-ramdisk already installed at recorded version + config. Skipping."
    return 0
  fi
  ...
  compute_ram_plan
  log_info "Plan: zram ${ZRAM_SIZE} ${ZRAM_ALGO} (model: ${DETECTED_PI_MODEL}, DB ${DETECTED_DB_MB}M, rotation ${WEEWX_DB_ROTATIONS})."
  ...
  log_ok "weewx-database-ramdisk installed."
...
do_uninstall() {
  case "$(status_state "$II_ID")" in
    uninstalled)
      log_info "weewx-database-ramdisk already uninstalled."
      echo -e "[  \e[0;32mOK\e[0m  ] weewx-database-ramdisk is already uninstalled."
      return 0
      ;;
    "")
      log_warn "No install record for weewx-database-ramdisk; nothing to revert."
  ...
  status_mark_uninstalled "$II_ID"
  log_ok "weewx-database-ramdisk uninstalled."
  echo -e "[  \e[0;32mOK\e[0m  ] weewx-database-ramdisk uninstalled (DB restored from SD-card mirror)."
  return 0
}
