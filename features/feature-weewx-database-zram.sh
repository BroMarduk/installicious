...
# Generated by feature-weewx-database-zram.sh for ${DETECTED_PI_MODEL}
...
do_install() {
  if status_should_skip "$II_ID" "$II_VERSION" "$FILE_CONFIG_WEEWX"; then
    log_info "weewx-database-zram already installed at recorded version + config. Skipping."
    return 0
  fi
  ...
  log_ok "weewx-database-zram installed."
...
do_uninstall() {
  case "$(status_state "$II_ID")" in
    uninstalled)
      log_info "weewx-database-zram already uninstalled."
      echo -e "[  \e[0;32mOK\e[0m  ] weewx-database-zram is already uninstalled."
      return 0
      ;;
    "")
      log_warn "No install record for weewx-database-zram; nothing to revert."
  ...
  status_mark_uninstalled "$II_ID"
  log_ok "weewx-database-zram uninstalled."
  echo -e "[  \e[0;32mOK\e[0m  ] weewx-database-zram uninstalled (DB restored from SD-card mirror)."
  return 0
}
