...
#
# *.override is also excluded: those are the user's personal override
# files (e.g. overrides/weewx.override) — gitignored, so they're never
# in the downloaded source tree. Without this exclude, `--delete` would
# wipe them on every re-sync. The exclude protects them in BOTH
# directions: rsync neither copies nor deletes them, so they persist
# untouched at $DEST exactly like state/menu-config.sh does.
