...
# DEFAULT lists the five weewx-role-specific features FIRST (weewx-setup +
# webroot + site-ram + database-ram + neowx-material — so the user sees
# the heavy weewx decisions before the general toggles) then
# locale/bash/motd/skyfield/ram-logging; OPTIONAL is Pi-tuning toggles.
# motd-weather and motd-updates are intentionally absent — both are
# hidden children of motd and would duplicate on the menu screens if
# listed at the role tier level.
