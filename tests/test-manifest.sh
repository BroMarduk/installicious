...
chkeq "real manifests" "$real_ids" "apache,bash,caddy,compressed-swap,git,jshon,lighttpd,locale,log2ram,motd,motd-updates,motd-weather,neowx-material,nginx,pip,pkupd,ram-logging,rconf,skyfield,webserver,webserver-ssl,webserver-under-construction,weewx,weewx-database-ram,weewx-setup,weewx-site-ram,weewx-webroot,zram,"
